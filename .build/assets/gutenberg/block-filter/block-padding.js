...
const addPaddingControl = createHigherOrderComponent(BlockEdit => {
    return props => {
        ...
        if (props.attributes.className) {
            ...
            Object.keys(paddingControlOptions).map(key => {
                classes = classes.filter(function (value, index, arr) {
                    return (
                        value !== 'has-block-vertical-padding--' + paddingControlOptions[key].value
                    );
                });
            });
        ...
        Object.keys(paddingControlOptions).map(key => {
            classNames.push({
                ['has-block-vertical-padding has-block-vertical-padding--' +
                paddingControlOptions[key].value]:
                    paddingControlOptions[key].value === shtPadding ? true : false,
            });
        });
        ...
        return (
            <Fragment>
                ...
                <BlockEdit {...props} />
                <InspectorControls>
                    <PanelBody title={__('Innenabstände', 'sht')} initialOpen={false}>
                        <div className='components-base-control'>
                            <label class='components-base-control__label'>
                                {__('Vertikaler Innenabstand ändern', 'sht')}
                            </label>
                            <ButtonGroup>
                                {Object.keys(paddingControlOptions).map(key => {
                                    return (
                                        <Button
                                            isSecondary={
                                                paddingControlOptions[key].value !== shtPadding
                                            }
                                            isPrimary={
                                                paddingControlOptions[key].value === shtPadding
                                            }
                                        ...
                                        >
                                            {paddingControlOptions[key].label}
                                        </Button>
                                    );
                                })}
                            </ButtonGroup>
