...
const addFontSizeControl = createHigherOrderComponent(BlockEdit => {
    return props => {
        ...
        if (props.attributes.className) {
            ...
            Object.keys(fontSizeControlOptions).map(key => {
                classes = classes.filter(function (value, index, arr) {
                    return value !== 'has-font-size-' + fontSizeControlOptions[key].value;
                });
            });
        ...
        Object.keys(fontSizeControlOptions).map(key => {
            classNames.push({
                ['has-font-size-' + fontSizeControlOptions[key].value]:
                    fontSizeControlOptions[key].value === shtFontSize ? true : false,
            });
        });
        ...
        return (
            <Fragment>
                ...
                <BlockEdit {...props} />
                <InspectorControls>
                    <PanelBody title={__('Schriftgrösse ändern', 'sht')} initialOpen={false}>
                        <div className='components-base-control'>
                            <label class='components-base-control__label'>
                                {__('Schriftgrösse ändern', 'sht')}
                            </label>
                            <ButtonGroup>
                                {Object.keys(fontSizeControlOptions).map(key => {
                                    return (
                                        <Button
                                            isSecondary={
                                                fontSizeControlOptions[key].value !== shtFontSize
                                            }
                                            isPrimary={
                                                fontSizeControlOptions[key].value === shtFontSize
                                            }
                                        ...
                                        >
                                            {fontSizeControlOptions[key].label}
                                        </Button>
                                    );
                                })}
                            </ButtonGroup>
